    ...
    <div>
      ...
      <div className="rounded-xl border border-[#E4DACA] bg-[#FBF8F2] p-3">
        ...
        <button
          ...
          className="mt-2 flex h-[36px] w-full items-center justify-center gap-1.5 rounded-lg bg-[#5C1626] text-[13px] font-medium text-white transition hover:opacity-90 disabled:opacity-50"
        >
          <Plus size={15} />
      ...
      <div className="mt-3">
        {isLoading && (
          <p className="py-3 text-center text-[13px] text-[#B08C7A]">
            Loading sizes…
          </p>
        )}
        ...
        {variants.map((v) => (
          ...
          >
            ...
            <span className="w-14 font-semibold text-[#3A1620]">
              {v.size?.name}
            </span>
            <span className="flex-1 text-[#8A7C64]">
              Stock: {v.stock}
            </span>
            <span className="font-mono text-[11px] text-[#B08C7A]">
              {v.sku}
            </span>
            <button
              type="button"
