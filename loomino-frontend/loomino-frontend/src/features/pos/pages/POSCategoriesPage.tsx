...
import { getCategories } from "../services/pos.service";
import DataTableShell from "../components/DataTableShell";
import CategoryFormModal from "../components/CategoryFormModal";
import type { Category, Paginated } from "../types/pos";
...
function POSCategoriesPage() {
  ...
  // Backend returns categories as a flat (unpaginated) list, since the
  // same endpoint feeds filter dropdowns elsewhere -- sliced client-side
  // here so this management page still gets DataTableShell's toolbar.
  const paged: Paginated<Category> = useMemo(() => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-[20px] font-bold text-[#221F35]">Categories</h1>
          <p className="text-[13px] text-[#726C8C]">Manage your categories</p>
        </div>
        <button
          type="button"
      ...
      <DataTableShell<Category>
        ...
        rowKey={(row) => row.id}
        emptyLabel="No categories yet — click Add to get started."
        filenameBase="categories"
        fetchAll={async () => (await getCategories()) as Category[]}
        columns={[
          { header: "Category", render: (row) => row.parent_name ? `${row.parent_name} › ${row.name}` : row.name },
      ...
      {addOpen && <CategoryFormModal mode="add" onClose={() => setAddOpen(false)} />}
      {editCategory && (
        <CategoryFormModal
          mode="edit"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl">
            ...
            <p className="mt-1 text-[13px] text-[#726C8C]">
              Products in this category won't be deleted — they'll just become uncategorized.
            </p>
