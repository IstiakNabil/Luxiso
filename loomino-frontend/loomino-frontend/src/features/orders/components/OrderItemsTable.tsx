...
/**
 * Order line items. #F6EFE8 header row, #CBCBCB borders,
...
function OrderItemsTable({ items }: OrderItemsTableProps) {
  return (
    <div className="border border-[#CBCBCB]">
      <div className="hidden grid-cols-[1fr_100px_120px_120px] gap-4 border-b border-[#CBCBCB] bg-[#F6EFE8] px-6 py-3 text-[14px] font-bold text-[#1F1210] md:grid">
        <span>Product</span>
      ...
      {items.map((item, index) => (
        ...
        >
          <div>
            <p className="text-[16px] font-bold capitalize text-[#1F1210]">
              {item.product_name}
            </p>
